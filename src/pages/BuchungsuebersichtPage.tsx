...
export default function BuchungsuebersichtPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Buchungsuebersicht[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Buchungsuebersicht | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getBuchungsuebersicht());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Buchungsuebersicht['fields']) {
  ...
  async function handleUpdate(fields: Buchungsuebersicht['fields']) {
    ...
    await LivingAppsService.updateBuchungsuebersichtEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Buchungsübersicht im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Buchungsübersicht suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('zimmernummer')}>
                <span className="inline-flex items-center gap-1">
                  Zimmernummer
                  {sortKey === 'zimmernummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/buchungsuebersicht/${record.record_id}`); }}>
                <TableCell className="font-medium">{record.fields.gast_vorname ?? '—'}</TableCell>
                <TableCell>{record.fields.gast_nachname ?? '—'}</TableCell>
                <TableCell>{record.fields.zimmernummer ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Buchungsübersicht. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
