...
import { BuchungsuebersichtViewDialog } from '@/components/dialogs/BuchungsuebersichtViewDialog';
...
const BUCHUNGSUEBERSICHT_FIELDS = [
  ...
  { key: 'buchungsstatus', label: 'Buchungsstatus', type: 'lookup/select', options: [{ key: 'bestaetigt', label: 'Bestätigt' }, { key: 'ausstehend', label: 'Ausstehend' }, { key: 'storniert', label: 'Storniert' }, { key: 'eingecheckt', label: 'Eingecheckt' }, { key: 'ausgecheckt', label: 'Ausgecheckt' }] },
  { key: 'hinweise', label: 'Hinweise', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'buchungsuebersicht', label: 'Buchungsübersicht', pascal: 'Buchungsuebersicht' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('buchungsuebersicht');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'buchungsuebersicht': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'buchungsuebersicht': return (data as any).buchungsuebersicht as Buchungsuebersicht[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'buchungsuebersicht': return BUCHUNGSUEBERSICHT_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'buchungsuebersicht': return {
        create: (fields: any) => LivingAppsService.createBuchungsuebersichtEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBuchungsuebersichtEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBuchungsuebersichtEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'buchungsuebersicht' || dialogState?.entity === 'buchungsuebersicht') && (
        <BuchungsuebersichtDialog
          open={createEntity === 'buchungsuebersicht' || dialogState?.entity === 'buchungsuebersicht'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'buchungsuebersicht' ? handleUpdate : (fields: any) => handleCreate('buchungsuebersicht', fields)}
          defaultValues={dialogState?.entity === 'buchungsuebersicht' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Buchungsuebersicht']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Buchungsuebersicht']}
        />
      )}
      {viewState?.entity === 'buchungsuebersicht' && (
        <BuchungsuebersichtViewDialog
          open={viewState?.entity === 'buchungsuebersicht'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'buchungsuebersicht', record: r }); }}
        />
      )}
