...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Buchungsuebersicht';
...
interface BuchungsuebersichtDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Buchungsuebersicht['fields']) => Promise<void>;
  defaultValues?: Buchungsuebersicht['fields'];
...
export function BuchungsuebersichtDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: BuchungsuebersichtDialogProps) {
  const [fields, setFields] = useState<Partial<Buchungsuebersicht['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Buchungsuebersicht['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'buchungsuebersicht');
      await onSubmit(clean as Buchungsuebersicht['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "gast_vorname": string | null, // Vorname des Gastes\n  "gast_nachname": string | null, // Nachname des Gastes\n  "zimmernummer": string | null, // Zimmernummer\n  "anreise": string | null, // YYYY-MM-DD\n  "abreise": string | null, // YYYY-MM-DD\n  "anzahl_personen": number | null, // Anzahl der Personen\n  "buchungsstatus": LookupValue | null, // Buchungsstatus (select one key: "bestaetigt" | "ausstehend" | "storniert" | "eingecheckt" | "ausgecheckt") mapping: bestaetigt=Bestätigt, ausstehend=Ausstehend, storniert=Storniert, eingecheckt=Eingecheckt, ausgecheckt=Ausgecheckt\n  "hinweise": string | null, // Hinweise\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Buchungsuebersicht['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Buchungsübersicht bearbeiten' : 'Buchungsübersicht hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'hinweise': (
      <div key="hinweise" className="space-y-1.5">
        ...
        <Textarea
          id="hinweise"
          placeholder="Allergien, Diäten, spezielle Wünsche..."
  ...
  const FIELD_LABELS: Record<string, string> = {"gast_vorname": "Vorname des Gastes", "gast_nachname": "Nachname des Gastes", "zimmernummer": "Zimmernummer", "anreise": "Anreisedatum", "abreise": "Abreisedatum", "anzahl_personen": "Anzahl der Personen", "buchungsstatus": "Buchungsstatus", "hinweise": "Hinweise"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.BUCHUNGSUEBERSICHT} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
