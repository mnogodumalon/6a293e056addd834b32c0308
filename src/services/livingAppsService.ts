...
import type { Buchungsuebersicht, CreateBuchungsuebersicht } from '@/types/app';
...
export class LivingAppsService {
  // --- BUCHUNGSUEBERSICHT ---
  static async getBuchungsuebersicht(): Promise<Buchungsuebersicht[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BUCHUNGSUEBERSICHT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Buchungsuebersicht[];
    return enrichLookupFields(records, 'buchungsuebersicht');
  }
  static async getBuchungsuebersichtEntry(id: string): Promise<Buchungsuebersicht | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BUCHUNGSUEBERSICHT}/records/${id}`);
    const record = { record_id: data.id, ...data } as Buchungsuebersicht;
    return enrichLookupFields([record], 'buchungsuebersicht')[0];
  }
  static async createBuchungsuebersichtEntry(fields: CreateBuchungsuebersicht) {
    return callApi('POST', `/apps/${APP_IDS.BUCHUNGSUEBERSICHT}/records`, { fields: cleanFieldsForApi(fields as any, 'buchungsuebersicht') });
  }
  static async updateBuchungsuebersichtEntry(id: string, fields: Partial<CreateBuchungsuebersicht>) {
    return callApi('PATCH', `/apps/${APP_IDS.BUCHUNGSUEBERSICHT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'buchungsuebersicht') });
  }
  static async deleteBuchungsuebersichtEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BUCHUNGSUEBERSICHT}/records/${id}`);
  }
